					...
					int pull)
{
	...
	rc = dbg_smblib_masked_write(chg, BATIF_ADC_INTERNAL_PULL_UP_REG,
				mask, val);
	if (rc < 0)
		dev_err(chg->dev,
...
static int smb5_parse_sdam(struct smb5 *chip, struct device_node *node)
{
	...
	for_each_available_child_of_node(node, child) {
		of_property_for_each_string(child, "reg", prop, name) {
			...
			rc = dbg_smblib_read(chg, base + PERPH_TYPE_OFFSET, &type);
			if (rc < 0) {
			...
			case SDAM_TYPE:
				...
				break;
			default:
				pr_info("%s ic type=0x%x base=0x%x, name=%s", __func__, type, base, name);
				break;
			}
		}
	}
			     ...
			     int val)
{
	...
	for (i = 0; i < ARRAY_SIZE(clamp_config.reg); i++) {
		rc = dbg_smblib_write(chg, clamp_levels[level].reg[i],
...
static int smb5_configure_typec(struct smb_charger *chg)
{
	...
	rc = dbg_smblib_read(chg, LEGACY_CABLE_STATUS_REG, &value);
	if (rc < 0) {
	...
	rc = dbg_smblib_read(chg, TYPE_C_SNK_STATUS_REG, &value);
	if (rc < 0) {
	...
	if (!(value & SNK_DAM_MASK)) {
		rc = dbg_smblib_masked_write(chg, TYPE_C_CFG_REG,
	...
	rc = dbg_smblib_write(chg, TYPE_C_INTERRUPT_EN_CFG_1_REG, 0);
	if (rc < 0) {
		dev_err(chg->dev,
	...
	rc = dbg_smblib_write(chg, TYPE_C_INTERRUPT_EN_CFG_2_REG,
	...
	if (chg->chg_param.smb_version != PMI632) {
		 ...
		 */
		rc = dbg_smblib_masked_write(chg, DEBUG_ACCESS_SRC_CFG_REG,
...
static int smb5_configure_iterm_thresholds_adc(struct smb5 *chip)
{
	...
	if (chip->dt.term_current_thresh_hi_ma) {
		...
		rc = dbg_smblib_batch_write(chg, CHGR_ADC_ITERM_UP_THD_MSB_REG,
				(u8 *)&raw_hi_thresh, 2);
		if (rc < 0) {
	...
	if (chip->dt.term_current_thresh_lo_ma) {
		...
		rc = dbg_smblib_batch_write(chg, CHGR_ADC_ITERM_LO_THD_MSB_REG,
				(u8 *)&raw_lo_thresh, 2);
		if (rc < 0) {
...
static int smb5_configure_iterm_thresholds(struct smb5 *chip)
{
	...
	case ITERM_SRC_ADC:
		if (chip->chg.chg_param.smb_version == PM8150B) {
			rc = dbg_smblib_masked_write(chg, CHGR_ADC_TERM_CFG_REG,
...
static int smb5_configure_mitigation(struct smb_charger *chg)
{
	...
	if (!chg->hw_die_temp_mitigation && !chg->hw_connector_mitigation &&
		...
		rc = dbg_smblib_masked_write(chg, BATIF_ADC_CHANNEL_EN_REG,
	...
	rc = dbg_smblib_masked_write(chg, MISC_THERMREG_SRC_CFG_REG,
...
static int smb5_init_dc_peripheral(struct smb_charger *chg)
{
	...
	rc = dbg_smblib_masked_write(chg, DCIN_LOAD_CFG_REG,
...
static int smb5_configure_recharging(struct smb5 *chip)
{
	...
	rc = dbg_smblib_masked_write(chg, CHGR_CFG2_REG, RECHG_MASK,
				(chip->dt.auto_recharge_vbat_mv != -EINVAL) ?
	...
	if (chip->dt.auto_recharge_vbat_mv != -EINVAL) {
		...
		rc = dbg_smblib_masked_write(chg, CHGR_NO_SAMPLE_TERM_RCHG_CFG_REG,
...
static int smb5_init_connector_type(struct smb_charger *chg)
{
	 ...
	 */
	if (chg->chg_param.smb_version == PMI632) {
		rc = dbg_smblib_read(chg, PMI632_TYPEC_MICRO_USB_MODE_REG, &val);
		if (rc < 0) {
	 ...
	 */
	if (!type) {
		rc = dbg_smblib_read(chg, TYPEC_U_USB_CFG_REG, &val);
		if (rc < 0) {
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if (chg->wa_flags & SW_THERM_REGULATION_WA) {
		rc = dbg_smblib_write(chg, MISC_THERMREG_SRC_CFG_REG,
	...
	rc = dbg_smblib_write(chg, AICL_RERUN_TIME_CFG_REG,
	...
	rc = dbg_smblib_masked_write(chg, DCDC_OTG_CFG_REG, OTG_EN_SRC_CFG_BIT, 0);
	if (rc < 0) {
		dev_err(chg->dev,
	...
	rc = dbg_smblib_masked_write(chg, WD_CFG_REG,
	...
	case 50:
		rc = dbg_smblib_masked_write(chg, CHARGE_INHIBIT_THRESHOLD_CFG_REG,
	...
	rc = dbg_smblib_write(chg, CHGR_FAST_CHARGE_SAFETY_TIMER_CFG_REG,
	...
	rc = dbg_smblib_masked_write(chg, DCDC_ENG_SDCDC_CFG5_REG,
...
#define pr1 pr_info("%s %d", __func__, __LINE__);
...
static void smb5_shutdown(struct platform_device *pdev)
{
	...
	dbg_smblib_masked_write(chg, MISC_SMB_EN_CMD_REG, EN_CP_CMD_BIT, 0);

	/* configure power role for UFP */
	if (chg->connector_type == QTI_POWER_SUPPLY_CONNECTOR_TYPEC)
		dbg_smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
