...
irqreturn_t smb5_schgm_flash_ilim2_irq_handler(int irq, void *data)
{
	...
	rc = dbg_smblib_write(chg, SCHGM_FLASH_S2_LATCH_RESET_CMD_REG,
...
irqreturn_t smb5_schgm_flash_state_change_irq_handler(int irq, void *data)
{
	...
	rc = dbg_smblib_read(chg, SCHGM_FLASH_STATUS_3_REG, &reg);
	if (rc < 0)
...
int schgm_flash_get_vreg_ok(struct smb_charger *chg, int *val)
{
	...
	rc = dbg_smblib_read(chg, SCHGM_FLASH_STATUS_2_REG, &stat);
	if (rc < 0) {
	...
	if (!vreg_state) {
		rc = dbg_smblib_read(chg, SCHGM_FLASH_STATUS_3_REG, &stat);
		if (rc < 0) {
		...
		if ((stat & FLASH_STATE_MASK) == FLASH_ERROR_VAL) {
			vreg_state = -EFAULT;
			rc = dbg_smblib_read(chg, SCHGM_FLASH_STATUS_5_REG,
					&stat);
			if (rc < 0) {
...
void schgm_flash_torch_priority(struct smb_charger *chg, enum torch_mode mode)
{
	...
	reg = mode;
	rc = dbg_smblib_masked_write(chg, SCHGM_TORCH_PRIORITY_CONTROL_REG,
...
int schgm_flash_init(struct smb_charger *chg)
{
	...
	if (chg->flash_derating_soc != -EINVAL) {
		rc = dbg_smblib_write(chg, SCHGM_SOC_BASED_FLASH_DERATE_TH_CFG_REG,
	...
	if (chg->flash_disable_soc != -EINVAL) {
		rc = dbg_smblib_write(chg, SCHGM_SOC_BASED_FLASH_DISABLE_TH_CFG_REG,
	...
	if (chg->headroom_mode != -EINVAL) {
		...
		rc = dbg_smblib_write(chg, SCHGM_FORCE_BOOST_CONTROL, reg);
		if (rc < 0) {
		...
		rc = dbg_smblib_write(chg, SCHGM_TORCH_PRIORITY_CONTROL_REG, reg);
		if (rc < 0) {
	...
	if ((chg->flash_derating_soc != -EINVAL)
		...
		rc = dbg_smblib_read(chg, SCHGM_FLASH_CONTROL_REG, &reg);
		if (rc < 0) {
