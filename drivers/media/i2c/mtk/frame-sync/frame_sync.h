...
__weak void fs_sa_request_switch_master(unsigned int idx) {
}
#endif
 ...
 */
#if !defined(FS_UT)
__weak unsigned int FrameSyncInit(struct FrameSync **pframeSync, struct device *dev) {
	return 0;
}

__weak void FrameSyncUnInit(struct device *dev) {
	return;
}

#else // FS_UT
__weak unsigned int FrameSyncInit(struct FrameSync **framesync) {
	return 0;
}
