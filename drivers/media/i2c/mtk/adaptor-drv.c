					...
					struct v4l2_subdev_format *fmt)
{
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY)
		fmt->format = *v4l2_subdev_get_try_format(&ctx->sd, sd_state,
							  fmt->pad);
	else
				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	mutex_lock(&ctx->mutex);
	ret = __imgsensor_get_pad_format(ctx, sd_state, fmt);
	mutex_unlock(&ctx->mutex);
...
static int imgsensor_get_temp(struct thermal_zone_device *data, int *temperature)
{
